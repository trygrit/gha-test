...
- name: Comment on PR
  uses: trygrit/terraform-pull-request-commenter@v1
  with:
    commenter_type: plan
    commenter_input: plan_output.txt
    commenter_type_exitcode: ${{ steps.plan.outputs.exitcode }}
```
...
| `commenter_input`         | _required_  | The path to the file containing the comment to post.               |
...
| `TERRAFORM_WORKSPACE`   | _optional_  | Default: `default`. This is used to separate multiple comments on a pull request in a matrix run.                                                         |
...
Feel free to head over to the [Issues](https://github.com/trygrit/terraform-pull-request-commenter/issues) tab to see if the issue you're having has already been reported. If not, [open a new one](https://github.com/sheeeng/terraform-pull-request-commenter/issues/new) and be sure to include as much relevant information as possible, including code-samples, and a description of what you expect to be happening.
